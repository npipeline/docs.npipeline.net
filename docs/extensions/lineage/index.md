...
- **Logging**: Built-in [`LoggingPipelineLineageSink`](https://github.com/npipeline/NPipeline/blob/main/src/NPipeline.Extensions.Lineage/LoggingPipelineLineageSink.cs) for structured logging
...
Lineage data is collected safely across parallel and concurrent pipeline executions using thread-safe data structures. The [`LineageCollector`](https://github.com/npipeline/NPipeline/blob/main/src/NPipeline.Extensions.Lineage/LineageCollector.cs) uses `ConcurrentDictionary` for storage and fine-grained locking for individual trail updates.
